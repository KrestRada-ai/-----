...
export const DecorationsModal: React.FC<DecorationsModalProps> = ({
  ...
  // FIX: Check that inventory count is a positive number before filtering. `count` can be `undefined`.
  const inventoryItems = Object.entries(inventory).filter(([, count]) => count && count > 0);
